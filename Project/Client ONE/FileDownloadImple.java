...
public class FileDownloadImple extends UnicastRemoteObject implements
	...
	public FileDownloadImple() throws Exception {
		...
		FileClientApplicationMain.urlString = "rmi://"
				+ InetAddress.getLocalHost().toString().split("/")[1] + ":"
				+ port + "/ftpfile";
		FileInformationForm.appendText("Create Ulr @ "
				+ FileClientApplicationMain.urlString);
		urlHash = new Hashtable<String, ArrayList<String>>();
		try {
			Naming.bind("//:" + port + "/ftpfile", this);
		} catch (Exception e) {
